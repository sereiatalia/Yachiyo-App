...
export async function handleCommand(interaction) {
  ...
  if(name==='fishalmanac') {
    ...
    const sections=order.filter(rarity=>byRarity.has(rarity)).map(rarity=>{
      const row=byRarity.get(rarity);
      const label=FISH_RARITIES[rarity]?.label||rarity;
      return '**'+label+' Collection:** '+row.discovered+' discovered  •  '+row.caught+' caught';
    });
    const body=sections.length?sections.join(String.fromCharCode(10)+String.fromCharCode(10)):'*No discoveries yet. The tide is waiting for your first cast.*';
    const description=['╭─────────────── 𓆝 ───────────────╮',body,'╰─────────────── 𓆝 ───────────────╯'].join(String.fromCharCode(10));
    return interaction.reply({embeds:[new EmbedBuilder().setColor(0x8e7dff).setAuthor({name:'Yachiyo • Celestial Almanac',iconURL:interaction.client.user.displayAvatarURL()}).setTitle('📖 ✧ CELESTIAL ALMANAC ✧').setDescription(description).setFooter({text:'Every discovery is recorded in Yachiyo’s cosmic archive.'})]});
  }
  ...
  if(name==='fishrod') {
    try {
      ...
      return interaction.reply({embeds:[new EmbedBuilder().setColor(0xf3a6c7).setTitle('🎣 '+rod.tier.name).setDescription('Your fishing rod grows with every cosmic catch.').addFields({name:'✦ Level',value:'**'+rod.level+' / 10**',inline:true},{name:'🍀 Luck',value:'+'+rod.tier.luck+'%',inline:true},{name:'💎 Value',value:'+'+rod.tier.value+'%',inline:true},{name:'Next evolution',value:nextTier?'**'+nextTier.name+'** • '+nextTier.cost.toLocaleString()+' coins':'**Maximum tier reached**'}).setFooter({text:nextTier?'Evolve your rod when you are ready.':'Your rod has reached its celestial maximum.'})],components:[row]});
