...
export async function handleCommand(interaction) {
  ...
  const adminOnly=['economy-add','logs','confession-setup','introduction-setup','introduction-panel','introduction-reset','introduction-status','fish-setup','curse-setup','curse','warn','warnings','kick','ban','timeout','purge','lock','unlock','unban','untimeout','vc-join','bump-panel','ticket-role','server-info-setup','server-info-edit','server-info-banner','server-info-staff'];
  ...
  if (name === 'server-info') { const info=await getServerInfo(interaction.guildId); if(!info) return interaction.reply({content:'An admin can create the panel with `/server-info-setup`.',ephemeral:true}); const row=new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('server_info_staffs').setLabel('STAFFS').setStyle(ButtonStyle.Secondary).setEmoji('🌷'),new ButtonBuilder().setCustomId('server_info_profile').setLabel('YOUR PROFILE').setStyle(ButtonStyle.Primary).setEmoji('♡'),new ButtonBuilder().setCustomId('server_info_bot').setLabel('SERVER BOT').setStyle(ButtonStyle.Secondary).setEmoji('⭑')); return interaction.reply({embeds:[await interaction.client.createServerInfoEmbed(interaction.guild,info)],components:[row],ephemeral:true}); }
