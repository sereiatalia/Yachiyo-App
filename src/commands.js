...
export async function handleCommand(interaction) {
  ...
  if(name==='spam-setup') { const action=interaction.options.getString('action'); if(action==='status') { const settings=await getSpamSettings(interaction.guildId); return interaction.reply({content:settings.enabled?'✅ Spam protection is **active**: 10 messages within 1 second = 1 warning; 3 warnings = 10-minute timeout.':'Spam protection is currently **off**.',ephemeral:true}); } if(action==='reset') { const user=interaction.options.getUser('user'); if(!user) return interaction.reply({content:'Choose a member to reset.',ephemeral:true}); await resetSpamWarnings(interaction.guildId,user.id); return interaction.reply({content:'✅ Reset spam warnings for '+user+'.',ephemeral:true}); } const enabled=action==='on'; await setSpamEnabled(interaction.guildId,enabled); return interaction.reply({content:enabled?'✅ Spam protection activated server-wide. **10 messages in 1 second** gives a warning; **3 warnings** gives a **10-minute timeout**.':'✅ Spam protection deactivated.',ephemeral:true}); }
