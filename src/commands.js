...
import { setupServerInfo, getServerInfo, updateServerInfo, updateServerInfoBanner } from './services/serverInfoService.js';
...
export async function handleCommand(interaction) {
  ...
  const adminOnly=['economy-add','logs','confession-setup','introduction-setup','introduction-panel','introduction-reset','introduction-status','fish-setup','curse-setup','curse','warn','warnings','kick','ban','timeout','purge','lock','unlock','unban','untimeout','vc-join','bump-panel','ticket-role','server-info-setup','server-info-edit','server-info-banner'];
  ...
  if (name === 'server-info') { const info=await getServerInfo(interaction.guildId); if(!info) return interaction.reply({content:'An admin can create the panel with `/server-info-setup`.',ephemeral:true}); return interaction.reply({embeds:[await interaction.client.createServerInfoEmbed(interaction.guild,info)],ephemeral:true}); }
