...
export const commands = [
  ...
  ,new SlashCommandBuilder().setName('temp-vc-setup').setDescription('Post the Create your own VC panel.').setDefaultMemberPermissions(PermissionFlagsBits.Administrator).setDMPermission(false).addChannelOption(o=>o.setName('channel').setDescription('Text channel for the panel').setRequired(true)).addChannelOption(o=>o.setName('category').setDescription('Optional category for created voice channels').setRequired(false))
  ,new SlashCommandBuilder().setName('temp-vc-panel').setDescription('Refresh the Create your own VC panel.').setDefaultMemberPermissions(PermissionFlagsBits.Administrator).setDMPermission(false)
...
export async function handleCommand(interaction) {
  ...
  if(name==='temp-vc-setup') { const channel=interaction.options.getChannel('channel'), category=interaction.options.getChannel('category'); if(!channel?.isTextBased()) return interaction.reply({content:'Choose a text channel for the panel.',ephemeral:true}); if(category && category.type!==4) return interaction.reply({content:'Choose a server category, not a normal channel.',ephemeral:true}); await saveTempVoiceSettings(interaction.guildId,channel.id,category?.id); await interaction.reply({content:'✅ Create-your-own-VC panel saved.',ephemeral:true}); return interaction.client.emit('tempVoicePanelRefresh',interaction.guildId); }
  if(name==='temp-vc-panel') { if(!await getTempVoiceSettings(interaction.guildId)) return interaction.reply({content:'Run `/temp-vc-setup` first.',ephemeral:true}); await interaction.reply({content:'✅ Refreshing the temporary VC panel.',ephemeral:true}); return interaction.client.emit('tempVoicePanelRefresh',interaction.guildId); }
