...
export async function handleCommand(interaction) {
  ...
  if(name==='roblox-user') { await interaction.deferReply({ephemeral:true}); try { const profile=await resolveRobloxUser(interaction.options.getString('username')); await saveRobloxProfile(interaction.guildId,interaction.user.id,profile); const embed=new EmbedBuilder().setColor(0xf3a6c7).setAuthor({name:interaction.user.globalName||interaction.user.username,iconURL:interaction.user.displayAvatarURL()}).setTitle(profile.displayName).setURL('https://www.roblox.com/users/'+profile.id+'/profile').setDescription('**@'+profile.username+'**\nRoblox ID: `'+profile.id+'`').setFooter({text:'Yachiyo • Roblox profile saved'}); if(profile.avatarUrl) embed.setThumbnail(profile.avatarUrl); return interaction.editReply({content:'✅ Your Roblox profile has been saved.',embeds:[embed]}); } catch(error) { return interaction.editReply({content:'⚠️ '+error.message}); } }
  if(name==='roblox-checkuser') { const user=interaction.options.getUser('user'); const saved=await getRobloxProfile(interaction.guildId,user.id); if(!saved) return interaction.reply({content:user+' has not saved a Roblox username yet.',ephemeral:true}); await interaction.deferReply(); try { const profile=await resolveRobloxUser(saved.username); const embed=new EmbedBuilder().setColor(0xf3a6c7).setAuthor({name:user.globalName||user.username,iconURL:user.displayAvatarURL()}).setTitle(profile.displayName).setURL('https://www.roblox.com/users/'+profile.id+'/profile').setDescription('**@'+profile.username+'**\nRoblox ID: `'+profile.id+'`').setFooter({text:'Yachiyo • verified Roblox profile'}); if(profile.avatarUrl) embed.setThumbnail(profile.avatarUrl); return interaction.editReply({embeds:[embed]}); } catch(error) { return interaction.editReply({content:'⚠️ '+error.message}); } }
  ...
  if(name==='mlbb') { try { const uid=normalizeMlbbUid(interaction.options.getString('uid')); await saveMlbbProfile(interaction.guildId,interaction.user.id,uid); return interaction.reply({content:'✅ Your Mobile Legends UID has been saved.',embeds:[new EmbedBuilder().setColor(0x3d8ee8).setAuthor({name:interaction.user.globalName||interaction.user.username,iconURL:interaction.user.displayAvatarURL()}).setTitle('MOBILE LEGENDS: BANG BANG').setDescription('**Player UID:** `'+uid+'`').setFooter({text:'Yachiyo • MLBB profile saved'})],ephemeral:true}); } catch(error) { return interaction.reply({content:'⚠️ '+error.message,ephemeral:true}); } }
  if(name==='mlbb-checkuser') { const user=interaction.options.getUser('user'); const saved=await getMlbbProfile(interaction.guildId,user.id); if(!saved) return interaction.reply({content:user+' has not saved an MLBB UID yet.',ephemeral:true}); return interaction.reply({embeds:[new EmbedBuilder().setColor(0x3d8ee8).setAuthor({name:user.globalName||user.username,iconURL:user.displayAvatarURL()}).setTitle('MOBILE LEGENDS: BANG BANG').setDescription('**Player UID:** `'+saved.player_uid+'`').setFooter({text:'Yachiyo • saved MLBB profile'})]}); }
