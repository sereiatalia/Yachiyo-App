...
export async function getReactionRoleByMessage(messageId, emoji) { await ensureTables(); return (await query('SELECT p.*,o.role_id,o.emoji FROM reaction_role_panels p JOIN reaction_role_options o ON o.panel_id=p.id WHERE p.message_id=$1 AND o.emoji=$2',[messageId,emoji])).rows[0] ?? null; }
