...
export async function setAuditCategoryChannel(guildId, category, channelId) { await ensureGuild(guildId); await query(`UPDATE guild_settings SET audit_channels = COALESCE(audit_channels,'{}'::jsonb) || jsonb_build_object($1,$2), updated_at=NOW() WHERE guild_id=$3`, [category, channelId, guildId]); }
