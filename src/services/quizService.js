...
export async function getCurrentRound(sessionId) { return (await query(`SELECT * FROM quiz_rounds WHERE session_id=$1 AND winner_id IS NULL ORDER BY round_number DESC LIMIT 1`, [sessionId])).rows[0] ?? null; }
...
export async function answerQuiz(sessionId,round,userId,submittedAnswer) { const current=(await query(`SELECT * FROM quiz_rounds WHERE session_id=$1 AND round_number=$2 AND winner_id IS NULL`, [sessionId,round])).rows[0]; if(!current) return null; const normalize=value=>String(value??'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim(); const accepted=[current.answer,...(current.accepted_answers??[])].map(normalize); if(!accepted.includes(normalize(submittedAnswer))) return {wrong:true,answer:current.answer}; const r=await query(`UPDATE quiz_rounds SET winner_id=$3,answered_at=NOW() WHERE session_id=$1 AND round_number=$2 AND winner_id IS NULL RETURNING *`, [sessionId,round,userId]); if(!r.rows[0]) return null; await query(`UPDATE quiz_players SET score=score+$3,correct_answers=correct_answers+1 WHERE session_id=$1 AND user_id=$2`, [sessionId,userId,1]); return r.rows[0]; }
