...
export function findCountryTime(text) {
...
const languageOf = text => {
  ...
  if(/\b(ano|kumusta|kamusta|sino|paano|salamat|pwede|tulong|ka|ikaw|oras|saan|petsa)\b/i.test(text)) return 'tl';
  return 'en';
};
...
const findChannelAnswer = (message, guild) => {
  ...
  if(!choices.length) return group ? `I could not find a public channel named for **${group.label}**. Please ask a staff member. ♡` : 'Try one of the public chat channels, or ask a staff member for the best place. ♡';
...
export async function getOfflineBrainReply({ text, guild, user, member }) {
  ...
  if (has(message,'how old','server age','age of the server','ilang taon','kailan ginawa','when was the server created','server created','creation date')) { const timestamp=Math.floor(guild.createdTimestamp/1000); return `˚. ᵎᵎ **${guild.name}** was created on <t:${timestamp}:D> — it is **${ageText(guild.createdAt)}** old.`; }
  if (isTimeQuestion(text)) return directTime ?? `⭑.ᐟ ${say.time}`;
  if (has(message,'what date','current date','date today','anong petsa','petsa ngayon','today','fecha')) { const date=new Intl.DateTimeFormat('en-PH',{timeZone:process.env.SERVER_TIMEZONE || 'Asia/Manila',weekday:'long',year:'numeric',month:'long',day:'numeric'}).format(new Date()); return `⭑.ᐟ Today is **${date}**.`; }
