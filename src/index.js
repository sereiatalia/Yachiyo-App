...
client.on('interactionCreate', interaction => { if (interaction.isButton()) { const messages={fish_again:'Use `/fish` to cast another line.',fish_almanac:'The Fish Almanac interface is being prepared.',fish_inventory:'The fishing inventory interface is being prepared.'}; return interaction.reply({content:messages[interaction.customId]||'Yachiyo is processing that action.',ephemeral:true}); } if (interaction.isChatInputCommand()) handleCommand(interaction).catch(async e => { console.error(e); if (!interaction.replied) await interaction.reply({content:'Yachiyo encountered an error.',ephemeral:true}); }); });
